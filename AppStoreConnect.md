...
Captured against ten weeks of seeded training (`DemoData`, Debug-only, triggered
by the `-seedDemoData` launch argument the screenshot test passes). Regenerate on
an **erased** simulator — leftover state from a previous test run otherwise
leaks into the shot:
...
Notes:
- The seeded data is generated by running real sessions through `Progression`,
  so the weights, streaks, bests and PR badges on every screen agree with the
  history behind them. Overhead press is scripted to stall, which is what puts
  the amber row on Progress and the amber cells in the week strip.
- `00_first_run.png` is captured separately, without the seed.
